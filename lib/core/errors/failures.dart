...
class NetworkFailure extends Failure {
  const NetworkFailure([
    super.message = 'Không có kết nối mạng. Vui lòng kiểm tra Wi-Fi hoặc 4G.',
    String? code,
  ]) : super(code: code);
}
...
class InvalidOtpFailure extends Failure {
  const InvalidOtpFailure([
    super.message = 'Mã OTP không hợp lệ hoặc đã hết hạn.',
    String? code,
  ]) : super(code: code);
}
...
class ReservationFailure extends Failure {
  const ReservationFailure([
    super.message = 'Không thể giữ chỗ cho thực phẩm này.',
    String? code,
  ]) : super(code: code);
}
